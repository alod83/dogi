...
mysql -u root -e "USE $new_db; CREATE TABLE IF NOT EXISTS tabVIAF LIKE $old_db.tabVIAF;INSERT tabVIAF SELECT * FROM $old_db.tabVIAF;"; 
echo "Done"

# run viaf extractor to extract links
echo "Extracting new URLs from VIAF"
php viaf_extract_urls.php -o $old_db -n $new_db;
echo "Done"
